...
def main():
    ...
    for url in urls:
        ...
        while True:
            try:
                el = driver.find_element_by_xpath(xpaths.get('see_more_link'))
                # implicitly waiting 10 seconds for the element to become
                # available again. Then, we click it.
                el.click()
            except StaleElementReferenceException:
                # this will be raised when the element we try to `click()`
                # on no longer exists, because Facebook has removed it.
                # This means we've reached the end of the "See More" list.
                break
